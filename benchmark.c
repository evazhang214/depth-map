...
static bool benchmarkMatrix(int imageWidth, int imageHeight, int featureWidth, int featureHeight, int maximumDisplacement, double *sum)
{
...
int main(int argc, char** argv)
{
	...
	for (int i = 0; i < ARRAY_SIZE(tests); i++)
	{
		...
		if (!benchmarkMatrix(tests[i][0], tests[i][1], tests[i][2], tests[i][3], tests[i][4], &sum))
		{
	...
	for (int i = 0; i < ARRAY_SIZE(oddTests); i++)
	{
		...
		if (!benchmarkMatrix(oddTests[i][0], oddTests[i][1], oddTests[i][2], oddTests[i][3], oddTests[i][4], &sum))
		{
			printf("Error: optimized does not match naive.\r\n");
			returnResult = -1;
		}
